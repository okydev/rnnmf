...
class Data_Factory():
    ...
    def split_data(self, ratio, R):
        ...
        for i in range(R.shape[0]):
            ...
            user_rating = tf.convert_to_tensor(user_rating)
            user_rating = tf.random.shuffle(user_rating).numpy()
        ...
        print("Loop item...")
        Rcsc = R.tocsc().T
        print("Csc convert done!")
        for j in remain_item:
            # item_rating = R.tocsc().T[j].nonzero()[1]
            item_rating = Rcsc[j].nonzero()[1]
            # np.random.shuffle(item_rating)
            item_rating = tf.convert_to_tensor(item_rating)
            item_rating = tf.random.shuffle(item_rating).numpy()
        ...
        print("Shuffle remaining...")
        # random.shuffle(remain_rating_list)
        remain_rating_list = tf.convert_to_tensor(remain_rating_list)
        remain_rating_list = tf.random.shuffle(remain_rating_list).numpy().tolist()
        ...
        if num_addition < 0:
            ...
            print("Shuffle tmp test...")
            # random.shuffle(tmp_test)
            tmp_test = tf.convert_to_tensor(tmp_test)
            tmp_test = tf.random.shuffle(tmp_test).numpy().tolist()
            valid = tmp_test[::2]
            test = tmp_test[1::2]
