...
'''
import os; 
#os.environ['KERAS_BACKEND'] = 'theano'; #os.environ['THEANO_FLAGS'] = 'device=cuda,floatX=float32'; 
import argparse
import sys
...
parser.add_argument("-z", "--run_cnn_only", type=bool,
                    help="Run CNN Only (default: False)", default=False)
...
if do_preprocess:
    ...
    run_cnn_only = args.run_cnn_only
    ...
    if run_cnn_only:
        ...
        import numpy as np
        print("Prepare RNN Module")
        # print(CNN_X[:10])

        # convert to real sentences
        vocab_list = D_all['X_vocab']
        list_text = CNN_X[:10]
        sentences = [' '.join([vocab_list[x-1][0] for x in txt]) for txt in list_text]

        text_analysis_module = RNN_module(dimension, vocab_size, 0.2,
                                emb_dim, 300, num_kernel_per_ws, init_W=init_W)
        # text_analysis_module.load_model(res_dir + '/CNN_weights.hdf5')
        text_analysis_module.model.summary()
        rnn_out = text_analysis_module.model.get_layer('sequential_1')
        rnn_out.summary()
        intermediate_layer_model = Model(inputs=text_analysis_module.model.input,
                                 outputs=rnn_out.get_output_at(-1))
        # text_analysis_module.model.get_layer('sequential_1').summary()
        intermediate_layer_model.compile(optimizer='rmsprop', loss='mse')
        intermediate_layer_model.summary()
        X_train = sequence.pad_sequences(CNN_X[:10], maxlen=300)
        Y = intermediate_layer_model.predict(
            {'input': X_train}, batch_size=len(X_train))
        with open(res_dir + '/calculated_rnn.txt', 'w') as f:
            for i in range(len(Y)):
                f.write('Sentence: ' + sentences[i]+'\r\n')
                f.write('Last RNN State: ' + np.array2string(Y[i], precision=8, separator=',',suppress_small=True).replace('\n', '           ')+'\r\n')
                # f.write('Filter 4: ' + np.array2string(Y[i][100:200], precision=8, separator=',',suppress_small=True).replace('\n', '           ')+'\r\n')
                # f.write('Filter 5: ' + np.array2string(Y[i][200:], precision=8, separator=',',suppress_small=True).replace('\n', '           ')+'\r\n')
        print("Done")
        exit(0)
